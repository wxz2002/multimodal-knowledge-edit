...
def generate_fast(
    ...
    with torch.no_grad():
        while input_ids.size(1) < max_out_len:  # while not exceeding max output length
            if 'llama' in model.name_or_path.lower() or 'baichuan' in model.name_or_path.lower():
                attention_mask = None
            elif 'opt' in model.name_or_path.lower() or 'vicuna' in model.name_or_path.lower():
                attention_mask = attention_mask
            else:
