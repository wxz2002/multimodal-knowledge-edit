...
from transformers import PreTrainedTokenizerBase, LlavaProcessor
...
class KnowledgeNeurons:
    def __init__(
        ...
        elif 'llava' == model_type:
            self.transformer_layers_attr = "language_model.model.layers"
            self.input_ff_attr = "mlp.gate_proj"
            self.output_ff_attr = "mlp.down_proj.weight"
            self.word_embeddings_attr = "language_model.model.embed_tokens.weight"
    ...
    def _prepare_inputs(self, prompt, target=None, encoded_input=None, image=None):
        if encoded_input is None:        
            if self.model_type == 'llava':
                encoded_input = self.processor(prompt, image, return_tensors='pt')
        ...
        if target is not None:
            if "qwen" in self.model_type or "gpt" in self.model_type or 't5' in self.model_type or 'llama' in self.model_type or 'llava' in self.model_type:
    ...
    def _generate(self, prompt, ground_truth):
        ...
        n_sampling_steps = len(target_label) if ("qwen" in self.model_type or "gpt" in self.model_type or 'llama' in self.model_type or 'llava' in self.model_type) else 1
        all_gt_probs = []
        all_argmax_probs = []
        argmax_tokens = []
    ...
    def get_scores_for_layer(
        ...
        if self.model_type == 'llava':
            encoded_input, mask_idx, target_label = self._prepare_inputs(
                prompt, ground_truth, encoded_input, image
            )   
        else:
            encoded_input, mask_idx, target_label = self._prepare_inputs(
        ...
        if attribution_method == "integrated_grads":
            ...
            for i in range(n_sampling_steps):
                if i > 0 and (self.model_type == "qwen" or self.model_type == "gpt" or self.model_type == 'llama' or self.model_type == 'llava'):
                    # retokenize new inputs
                    if self.model_type == 'llava':
                        encoded_input, mask_idx, target_label = self._prepare_inputs(
                            prompt, ground_truth, image=image
                        )
                    else :
                        encoded_input, mask_idx, target_label = self._prepare_inputs(
                            prompt, ground_truth
