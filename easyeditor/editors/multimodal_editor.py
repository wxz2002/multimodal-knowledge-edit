...
from ..evaluate import (compute_icl_multimodal_edit_quality,
                        compute_multimodal_edit_quality, 
...
class MultimodalEditor:
    ...
    def __init__(self,
        ...
        if type(self.model_name) is str:
            if hparams.model_name == "blip2":
                ...
                # model = LlavaForConditionalGeneration.from_pretrained(hparams.name).to(hparams.device)

            self.model = model
            # Get tokenizer and vis_processor
            if hparams.model_class == "Blip2OPT":
                vis_processor = BlipImageEvalProcessor(image_size=364, mean=None, std=None)
            elif hparams.model_class == "LLAVA":
                vis_processor = transformers.CLIPImageProcessor.from_pretrained(hparams.clip_name)
            else:
    ...
    def edit_dataset(self,
        ...
        assert self.alg_name in ['IKE', 'ROME'] or print('Only IKE and ROME supported for MultimodalEditor')
        num_edits = 1
        ...
        if self.alg_name == 'ROME':
            if self.model_name == "blip2":
                language_model = self.model.opt_model
                ...
                language_model.lm_head = self.model.lm_head
        ...
        for i, request in enumerate(tqdm(ds, desc='Editing dataset', total=len(ds))):
            ...
            if self.alg_name == 'IKE':
                ...
                if 'image_locality_output' in metrics['post'].keys():
                    assert len(metrics['post']['image_locality_output']) == \
                            len(metrics['pre']['image_locality_output'])
                    base_image_logits = metrics['pre']['image_locality_output'].to(torch.float32)
                    post_image_logits = metrics['post']['image_locality_output'].to(torch.float32)
                    if post_image_logits.shape[1] > base_image_logits.shape[1]:
                        post_image_logits = post_image_logits[:, -base_image_logits.shape[1]:, :]
                    else:
                        base_image_logits = base_image_logits[:, -post_image_logits.shape[1]:, :]
                    ...
                    base_image_logits_softmax_top_k = torch.topk(torch.nn.functional.softmax(base_image_logits, dim=-1), k=10, dim=-1).indices
                    post_image_base_logits_softmax_top_k = torch.topk(torch.nn.functional.softmax(post_image_logits, dim=-1), k=10, dim=-1).indices
                    metrics['post']['image_locality_acc'] = sum(post_image_base_logits_softmax_top_k.view(-1) == base_image_logits_softmax_top_k.view(-1))/post_image_base_logits_softmax_top_k.view(-1).shape[0]
                    metrics['post'].pop('image_locality_output')
                    metrics['pre'].pop('image_locality_output')
            elif self.alg_name == 'ROME':
                metrics = {}
                edited_model = copy.deepcopy(self.model).to(self.hparams.device)
                metrics['pre'] = compute_multimodal_edit_results(edited_model, self.model_name, self.hparams, self.tok, request, self.hparams.device)
                rome_request = self.prepare_inputs_for_rome(request)
                edited_language_model, weights_copy = self.apply_algo(
                    language_model,
                    self.tok,
                    rome_request,
                    self.hparams,
                ...
                if self.model_name == "blip2":
                    edited_model.opt_model = edited_language_model
                elif self.model_name == "minigpt4":
                    edited_model.llama_model = edited_language_model
                elif self.model_name == "llava":
                    edited_model.model.embed_tokens = edited_language_model.model.embed_tokens
                    edited_model.model.layers = edited_language_model.model.layers
                    edited_model.model.norm = edited_language_model.model.norm
                    edited_model.lm_head = edited_language_model.lm_head
                ...
                metrics['post'] = compute_multimodal_edit_results(edited_model, self.model_name, self.hparams, self.tok,
                                                    request, self.hparams.device)
            ...
            if verbose:
                # LOG.info(
                #     f"{i} editing: {request['question']} -> {request['answer']}  \n {metrics}"
                # )
    ...
    def prepare_inputs_for_rome(self,request):
        rome_request = {
            'prompt': request['original_question'],
            'target_new': request['answer'],
