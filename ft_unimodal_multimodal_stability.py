...
def get_kn_neurons(model, data, image_path, tokenizer, processor, device, ft_mode):
    ...
    stability_b_to_c_neurons = kn.get_coarse_neurons(prompt=stability_b_to_c_prompt, ground_truth=data['knowledge_edit']['answer_new'],
                                                        batch_size=1, steps=20, adaptive_threshold=0.15, image=stability_b_to_c_image, use_target_label=use_target_label)
    ...
    stability_a_to_c_neurons = kn.get_coarse_neurons(prompt=stability_a_to_c_prompt, ground_truth=data['knowledge_edit']['answer_new'],
                                                batch_size=1, steps=20, adaptive_threshold=0.15, image=stability_a_to_c_image, use_target_label=use_target_label)
...
if __name__ == "__main__":
    ...
    for i, batch in tqdm(enumerate(val_loader)):
        ...
        if data[i]['subject'] not in ft_unimodal_subjects:
            ...
            ft_a_to_b_neurons, ft_b_to_c_neurons, ft_a_to_c_neurons, ft_stability_a_to_b_neurons, ft_stability_b_to_c_neurons, ft_stability_a_to_c_neurons = get_kn_neurons(copy_model, data[i], args.image_path, tokenizer, processor, device, "unimodal")
        ...
        if data[i]['subject'] not in ft_multimodal_subjects:
            ...
            ft_a_to_b_neurons, ft_b_to_c_neurons, ft_a_to_c_neurons, ft_stability_a_to_b_neurons, ft_stability_b_to_c_neurons, ft_stability_a_to_c_neurons = get_kn_neurons(copy_model, data[i], args.image_path, tokenizer, processor, device, "multimodal")
