    ...
    def validation_step(self, batch, batch_idx=None):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                ...
                labels = self.fmodel(batch['edit_inner'], params=params).labels
                results = multiclass_log_probs(self.model_hparams, logits, labels, shift=True, exact_match=self.model_hparams.exact_match, multimodal=True)
                ...
                labels = self.fmodel(batch['edit_outer'], params=params).labels
                results = multiclass_log_probs(self.model_hparams, logits, labels, shift=True, exact_match=self.model_hparams.exact_match, multimodal=True)
                ...
                labels = self.fmodel(batch['edit_outer_image'], params=params).labels
                results = multiclass_log_probs(self.model_hparams, logits, labels, shift=True, exact_match=self.model_hparams.exact_match, multimodal=True)
